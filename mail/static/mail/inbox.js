document.addEventListener('DOMContentLoaded', function() {
  ...
  //event handler for a submit button
...
function view_email(id){
      ...
      .then(email => {
        ...
        //Archive and unachirve functions
...
function load_mailbox(mailbox) {
  ...
  document.querySelector('#emails-view').innerHTML = `<h3>${mailbox.charAt(0).toUpperCase() + mailbox.slice(1)}</h3>`;
      ...
      //Fetching emails for the mailbox and user
      fetch(`/emails/${mailbox}`)
        ...
        emails.forEach(singleEmail => {
              ...
              newEmail.innerHTML = `<h4>Sender: ${singleEmail.sender}</h4>
                                    <h5>Subject: ${singleEmail.subject}</h5>
                                    <P>${singleEmail.timestamp}</p>
                                    `;
              //Backgroud color of emails
